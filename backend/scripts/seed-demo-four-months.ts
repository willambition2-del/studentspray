...
export async function seedDemoFourMonths() {
  ...
  // 1. Idempotency Check & Atomic Clean of Demo Forum if it already exists
  ...
  // Track accounts for credentials file
  const credentialsList: { role: string; username: string; name: string; info: string }[] = [];

  // 2. Create Forum
  console.log('\n🏛️ Step 1: Creating Demo Forum: ملتقى القرآن التجريبي...');
  ...
  // 3. Create 3 Branches
  console.log('🏢 Step 2: Creating 3 Branches...');
  ...
  console.log('🔑 Step 3: Setting up Role and Permission Catalog...');
  ...
  // 5. Create Management Accounts (GM, Exec)
  console.log('👤 Step 4: Creating Management Accounts (GM, Exec)...');
  ...
  // 6. Create 2 Technical Supervisors
  console.log('🧐 Step 5: Creating 2 Technical Supervisors...');
  ...
  // 7. Create 10 Teachers
  console.log('👨‍🏫 Step 6: Creating 10 Teachers and Profiles...');
  const teacherDefs = [
  ...
  // 8. Create 10 Halaqas across the 3 Branches
  console.log('📖 Step 7: Creating 10 Halaqas with Teacher and Supervisor Assignments...');
  const halaqaDefs = [
  ...
  // 9. Create 150 Students & ~130 Parents
  console.log('🎓 Step 8: Creating 150 Students and 130 Parents with Family Linkages...');
  const studentUsers: any[] = [];
  const studentProfiles: any[] = [];
  const parentUsers: any[] = [];
  const parentProfiles: any[] = [];

  // Distribution: 112 parents with 1 child, 16 parents with 2 children, 2 parents with 3 children = 112 + 32 + 6 = 150
  ...
  for (const childCount of familyConfigs) {
    ...
    for (let c = 0; c < childCount; c++) {
      ...
      // Persona generation: 0 = High achiever (25%), 1 = Average (55%), 2 = Needs attention (20%)
  ...
  // 10. Distribute 150 Students into 10 Halaqas
  console.log('📋 Step 9: Distributing 150 Students across 10 Halaqas...');
  let studentOffset = 0;
  ...
  // 11. Create Academic Year and Terms
  console.log('📅 Step 10: Creating Academic Year and Terms...');
  ...
  // 12. Create Educational Plans for all 10 Halaqas
  console.log('📐 Step 11: Creating Educational Plans and Items...');
  ...
  // 13. Generate 4 Months of Daily Attendance Sessions and Records
  console.log('⏱️ Step 12: Generating 4 Months of Attendance Sessions & Records (excluding Fridays)...');
  const sessionDates: Date[] = [];
  const currentDate = new Date(START_DATE);
  while (currentDate <= END_DATE) {
    // Exclude Fridays (Day 5 in JS Date: 0=Sun, 5=Fri, 6=Sat)
  ...
  console.log(`  -> Generating ${sessionDates.length} operational session dates across 10 halaqas...`);

  // Batch insert attendance sessions and records
  ...
  // Fetch created sessions to get their IDs and map them
  ...
  for (const s of createdSessions) {
    ...
    for (const st of students) {
      ...
      // Realistic persona-based attendance probabilities
      const rand = rng.next();
      if (st.persona === 0) {
        // High achiever: 98% present
      ...
      // If today, ensure high overall presence (~92%)
      if (isToday) {
      ...
      // If Present, add Memorization and Revision records (sampled across days)
      if (status === AttendanceStatus.PRESENT) {
        const surah = rng.choice(SURAHS);
        const fromAyah = 1;
        const toAyah = Math.min(surah.ayahs, rng.nextInt(5, 20));

        // Memorization: ~50% of days attended
  ...
  // 14. Create 25 Exams across terms and halaqas
  console.log('📝 Step 13: Creating 25 Exams, Criteria, and ~350 Exam Results...');
  const examTitles = [
  ...
  for (let i = 0; i < 25; i++) {
    ...
    // Assign 2 upcoming scheduled exams explicitly to demo_teacher's halaqa (halaqas[0])
    ...
    // Create 2 criteria with Prisma Decimal max scores (60 + 40 = 100)
    ...
    // If completed exam, generate exam results for students in this halaqa
    if (!isUpcoming) {
  ...
  // 15. Create Student Evaluations across 4 Months
  console.log('🌟 Step 14: Creating ~450 Student Evaluations...');
  const evalNotes = [
  ...
  // 16. Create 15 Activities
  console.log('🏕️ Step 15: Creating 15 Activities & Participants...');
  const activityDefs = [
  ...
  // 17. Create 8 Competitions
  console.log('🏆 Step 16: Creating 8 Competitions & Published Results...');
  const compDefs = [
  ...
  // 18. Create 50 Awards
  console.log('🎖️ Step 17: Creating 50 Awards and Granting them to Students...');
  ...
  // 19. Create General Shelf (4 Sections, 50 Posts)
  console.log('📚 Step 18: Creating General Shelf (4 Sections, 50 Posts across 4 Months)...');
  ...
  // 20. Create 20 Chat Conversations & 300 Messages
  console.log('💬 Step 19: Creating 20 Chat Conversations and ~300 Messages with Unread States...');
  ...
  for (let i = 0; i < 20; i++) {
    ...
    for (const u of memberUsers) {
      ...
      // For demo_teacher in conversation 0 (halaqa group 0), leave last 3 messages unread
      if (i === 0 && u.id === teacherUsers[0].id) {
    ...
    // Add 15 messages per conversation
    for (let m = 0; m < 15; m++) {
      let msgDate = new Date(START_DATE.getTime() + ((i * 15 + m) / 300) * (END_DATE.getTime() - START_DATE.getTime()));
      let sender = memberUsers[m % memberUsers.length];

      // In conversation 0, ensure the last 3 messages are sent by GM / Exec after demo_teacher's lastReadAt
      if (i === 0 && m >= 12) {
        sender = m % 2 === 0 ? gmUser : execUser;
  ...
  // 21. Create ~400 Notifications
  console.log('🔔 Step 20: Creating Notifications with Read/Unread States...');
  const notifDefs = [
  ...
  // Explicitly ensure 5 recent unread notifications for demo_teacher
  ...
  // 22. Create 40 Administrative Tasks
  console.log('📋 Step 21: Creating 40 Administrative Tasks...');
  const taskTitles = [
  ...
  // 23. Create 25 Administrative Requests
  console.log('📑 Step 22: Creating 25 Administrative Requests...');
  const reqTitles = [
  ...
  // 24. Create 25 Supervisory Field Visits and Recommendations
  console.log('🔍 Step 23: Creating 25 Supervisory Field Visits & 20 Recommendations...');
  for (let i = 0; i < 25; i++) {
  ...
  // 25. Write Credentials File
  console.log('\n📄 Step 24: Writing Demo Credentials to backend/demo-four-months-accounts.txt...');
